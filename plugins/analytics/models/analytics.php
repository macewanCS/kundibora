...
class Analytics_Model extends Model {
    ...
    {
            $sql = "SELECT COUNT( category.id ) AS count, category.id, category.category_title, category.category_color, incident.incident_date FROM category JOIN incident_category ON category.id=incident_category.category_id JOIN incident ON incident_category.incident_id=incident.id WHERE category.id=".$category_id." GROUP BY incident.incident_date";
            return $this->db->query($sql);
    }
}
